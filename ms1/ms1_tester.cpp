...
const char instructionMessages[10][81] = {">>>>>Enter 0 and then 0 again",
                                          ">>>>>Enter 1",
                                          ">>>>>Enter 1 again",
                                          ">>>>>Enter 2",
                                          ">>>>>Enter 1",
...
void menuTester() {
  ...
  m2 << "Order more";
  if (m1) {
    cout << "The " << m1 << " is not empty and has " << (unsigned int)(m1)
         << " menu items." << endl;
  } else {
    cout << "Wrong output, your bool cast is done incorrenctly!";
  }
...
void dateTester() {
  Date D1, // daysSince0001_1_1 to be recieved from console
      D2(2024, 6, 10);
...
Date getDate() {
  ...
  } while (!D && cout << D.dateStatus()
                      << ", Please try again > "); // if D is invalid, print
